...
std::map<alljoyn_interfacedescription, WeakReference*> RoverConsumer::SourceInterfaces;

RoverConsumer::RoverConsumer(AllJoynBusAttachment^ busAttachment)
...
void RoverConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void RoverConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void RoverConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus RoverConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<RoverJoinSessionResult^>^ RoverConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ RoverWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> RoverJoinSessionResult^
    {
        auto result = ref new RoverJoinSessionResult();
        result->Consumer = ref new RoverConsumer(watcher->BusAttachment);
        result->Status = result->Consumer->JoinSession(serviceInfo);
        return result;
    });
}

IAsyncOperation<RoverLeftResult^>^ RoverConsumer::LeftAsync()
{
    return create_async([this]() -> RoverLeftResult^
    {
...
IAsyncOperation<RoverRightResult^>^ RoverConsumer::RightAsync()
{
    return create_async([this]() -> RoverRightResult^
    {
...
IAsyncOperation<RoverForwardResult^>^ RoverConsumer::ForwardAsync()
{
    return create_async([this]() -> RoverForwardResult^
    {
...
IAsyncOperation<RoverBackwardResult^>^ RoverConsumer::BackwardAsync()
{
    return create_async([this]() -> RoverBackwardResult^
    {
...
IAsyncOperation<RoverStopResult^>^ RoverConsumer::StopAsync()
{
    return create_async([this]() -> RoverStopResult^
    {
...
IAsyncOperation<RoverManualResult^>^ RoverConsumer::ManualAsync()
{
    return create_async([this]() -> RoverManualResult^
    {
...
void RoverConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
...
int32 RoverConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<RoverConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<RoverConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<RoverConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.alljoyn.example.Rover");
