...
std::map<alljoyn_interfacedescription, WeakReference*> RobotConsumer::SourceInterfaces;

RobotConsumer::RobotConsumer(AllJoynBusAttachment^ busAttachment)
...
void RobotConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void RobotConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void RobotConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus RobotConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<RobotJoinSessionResult^>^ RobotConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ RobotWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> RobotJoinSessionResult^
    {
        auto result = ref new RobotJoinSessionResult();
        result->Consumer = ref new RobotConsumer(watcher->BusAttachment);
        result->Status = result->Consumer->JoinSession(serviceInfo);
        return result;
    });
}

IAsyncOperation<RobotLeftResult^>^ RobotConsumer::LeftAsync()
{
    return create_async([this]() -> RobotLeftResult^
    {
...
IAsyncOperation<RobotRightResult^>^ RobotConsumer::RightAsync()
{
    return create_async([this]() -> RobotRightResult^
    {
...
IAsyncOperation<RobotForwardResult^>^ RobotConsumer::ForwardAsync()
{
    return create_async([this]() -> RobotForwardResult^
    {
...
IAsyncOperation<RobotBackwardResult^>^ RobotConsumer::BackwardAsync()
{
    return create_async([this]() -> RobotBackwardResult^
    {
...
IAsyncOperation<RobotStopResult^>^ RobotConsumer::StopAsync()
{
    return create_async([this]() -> RobotStopResult^
    {
...
IAsyncOperation<RobotAutomaticResult^>^ RobotConsumer::AutomaticAsync()
{
    return create_async([this]() -> RobotAutomaticResult^
    {
...
IAsyncOperation<RobotManualResult^>^ RobotConsumer::ManualAsync()
{
    return create_async([this]() -> RobotManualResult^
    {
...
void RobotConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
...
int32 RobotConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<RobotConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<RobotConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<RobotConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.alljoyn.example.Robot");
